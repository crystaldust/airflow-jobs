...
from oss_know.libs.base_dict.variable_key import OPENSEARCH_SYNC_INTERVAL, OPENSEARCH_SYNC_COMBINATION_TYPE, \
    SYNC_FROM_OPENSEARCH_CONN_INFO, OPENSEARCH_CONN_DATA, SYNC_OPENSEARCH_GITHUB_ISSUES_COMMENTS_INCLUDES
...
all_owner_repos = Variable.get(SYNC_OPENSEARCH_GITHUB_ISSUES_COMMENTS_INCLUDES, deserialize_json=True, default_var=None)
...
with DAG(dag_id='github_issues_comments_sync_from_opensearch',  # schedule_interval='*/5 * * * *',
    ...
    for letter, owner_repos in task_groups_by_capital_letter.items():
        op_sync_github_issues_comments_from_opensearch_group = PythonOperator(
            task_id=f'op_sync_github_issues_comments_from_opensearch_group_{letter}',
            python_callable=do_sync_github_issues_comments_from_opensearch_by_group,
        ...
        if prev_group:
            prev_group >> op_sync_github_issues_comments_from_opensearch_group
        prev_group = op_sync_github_issues_comments_from_opensearch_group
