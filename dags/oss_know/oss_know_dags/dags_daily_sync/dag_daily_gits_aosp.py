...
from oss_know.libs.util.clickhouse import get_uniq_owner_repos_fixed_owner
...
from oss_know.libs.util.base import get_opensearch_client, arrange_owner_repo_into_letter_groups_by_repo
...
with DAG(dag_id='daily_gits_sync_AOSP',  # schedule_interval='*/5 * * * *',
    ...
    if not includes:
        ...
        uniq_owner_repos = get_uniq_owner_repos_fixed_owner(clickhouse_conn_info, 'gits', 'AOSP')
    else:
    ...
    task_groups_by_capital_letter = arrange_owner_repo_into_letter_groups_by_repo(uniq_owner_repos)
