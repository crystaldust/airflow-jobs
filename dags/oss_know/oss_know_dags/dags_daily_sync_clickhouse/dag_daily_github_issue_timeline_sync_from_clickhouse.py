...
from oss_know.libs.base_dict.variable_key import CLICKHOUSE_DRIVER_INFO, SYNC_FROM_CLICKHOUSE_DRIVER_INFO, \
    ...
    CLICKHOUSE_GITHUB_ISSUES_TIMELINE_SYNC_INTERVAL
...
sync_interval = Variable.get(CLICKHOUSE_GITHUB_ISSUES_TIMELINE_SYNC_INTERVAL, default_var=None)
if not sync_interval:
    sync_interval = Variable.get(CLICKHOUSE_SYNC_INTERVAL, default_var=None)

# Daily sync github_issues_timeline data from other clickhouse environment by owner/repo
with DAG(dag_id='daily_github_issues_timeline_sync_from_clickhouse',  # schedule_interval='*/5 * * * *',
    ...
    op_init = PythonOperator(task_id='op_init_daily_github_issues_timeline_sync_from_clickhouse',
                             python_callable=do_init)


    def do_sync_github_issues_timeline_from_clickhouse_by_group(params):
    ...
    for letter, owner_repos in task_groups_by_capital_letter.items():
        op_sync_github_issues_timeline_from_clickhouse_group = PythonOperator(
            task_id=f'op_sync_github_issues_timeline_from_clickhouse_group_{letter}',
            python_callable=do_sync_github_issues_timeline_from_clickhouse_by_group,
        ...
        if not prev_group:
            op_init >> op_sync_github_issues_timeline_from_clickhouse_group
        else:
            prev_group >> op_sync_github_issues_timeline_from_clickhouse_group
        prev_group = op_sync_github_issues_timeline_from_clickhouse_group
