...
with DAG(dag_id='daily_github_prs_sync_from_clickhouse',  # schedule_interval='*/5 * * * *',
    ...
    all_owner_repos = Variable.get(DAILY_SYNC_CLICKHOUSE_GITHUB_PRS_INCLUDES, deserialize_json=True,
                                   default_var=None)
    ...
    def do_init():
        logger.info(f"Start init_daily_github_prs_sync_from_clickhouse({sync_combination_type}), {all_owner_repos}")
