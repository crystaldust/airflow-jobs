...
from oss_know.libs.base_dict.variable_key import CLICKHOUSE_DRIVER_INFO, SYNC_FROM_CLICKHOUSE_DRIVER_INFO, \
    CLICKHOUSE_SYNC_INTERVAL, CLICKHOUSE_SYNC_COMBINATION_TYPE, DAILY_SYNC_CLICKHOUSE_GITS_INCLUDES, \
    CLICKHOUSE_GITS_SYNC_INTERVAL
...
sync_interval = Variable.get(CLICKHOUSE_GITS_SYNC_INTERVAL, default_var=None)
if not sync_interval:
    Variable.get(CLICKHOUSE_SYNC_INTERVAL, default_var=None)
...
with DAG(dag_id='daily_gits_sync_from_clickhouse',  # schedule_interval='*/5 * * * *',
    ...
    all_owner_repos = Variable.get(DAILY_SYNC_CLICKHOUSE_GITS_INCLUDES, deserialize_json=True, default_var=None)
