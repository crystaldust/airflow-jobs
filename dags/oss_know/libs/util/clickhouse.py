...
def get_uniq_owner_repos_fixed_owner(clickhouse_conn_info, table_name, owner_name):
    sql = f'''
    select distinct search_key__owner, search_key__repo, search_key__origin from {table_name}
    where search_key__owner='{owner_name}'
    '''
    ck_client = CKServer(
        host=clickhouse_conn_info.get('HOST'),
        port=clickhouse_conn_info.get('PORT'),
    ...
    result = ck_client.execute_no_params(sql)
    distinct_owner_repos = list(map(lambda tup: {'owner': tup[0], 'repo': tup[1], 'origin': tup[2]}, result))
